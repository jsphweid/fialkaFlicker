	...
	constructor() {
		...
		this.sketch = function(p) {
			p.setup = function() {
				...
				// load video
				// vid = p.createVideo(['https://dl.dropboxusercontent.com/u/67477162/perm/vidsForSites/dance.mp4'])
				vid = p.createVideo([this.p5file]) // get vid
				// debugger;
			...
			p.draw = function() {
				...
				if (!_this.flick.flicker) {
					// if no flickering just draw the image and return
					p.image(vid, 0, 0)
					// debugger;
					return
				}

				if (_this.flick.drawNow === 0) {
					// if drawing black
					if (_this.flick.black > 0) {
						// and still drawing black
						p.background(0) // draw black
						_this.flick.black--
					} else {
						// but if reached the end
						_this.flick.drawNow = 1 // switch to draw white
						_this.flick.black = _this.flick.blackCount // reset
					}
				} else if (_this.flick.drawNow === 1) {
					// if drawing white
					if (_this.flick.white > 0) {
						// and still drawing white
